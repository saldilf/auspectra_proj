...
def parse_cmdline(argv):
    ...
    parser = ArgumentParser(
        description='This program reads data from xlsx file, analyzes it, plots it, and reports key parameters. '
                    'All program output is user-controlled. Please note that the commands below can be used in concert to achieve multiple desired outcomes. '
                    '')
...
def data_analysisNorm(data_file, plot = True):
    # read data from xlsx
...
def main(argv=None):
    ...
    if args['plot'] is True:
        ...
        if args['normalize'] is True:
            if args['table'] is True:
                print("You've normalized the data and generated a table but no plot")
                data = data_analysisNorm(args['workbook'], plot=False)
                df = pd.DataFrame(data)
                render_mpl_table(df, header_columns=0, col_width=3.0)
            else:
